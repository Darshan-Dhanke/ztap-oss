...
PROJECT="${1:-lake}"
TABLE="${2:-orders}"
SCHEMA="proj_${PROJECT}"                         # matches the Postgres schema name
FQ="delta.${SCHEMA}.${TABLE}"
...
docker exec ztap-trino trino --execute \
  "CALL delta.system.register_table(schema_name => '${SCHEMA}', table_name => '${TABLE}', table_location => '${LOC}')" \
  >/dev/null 2>&1 || true
